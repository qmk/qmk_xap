...
async xapEnabledSubsystemCapabilities(id: string) : Promise<Result<XapEnabledSubsystemCapabilitiesFlags, Error>> {
try {
    return { status: "ok", data: await TAURI_INVOKE("xap_enabled_subsystem_capabilities", { id }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async qmkConfigBlobChunk(id: string, arg: number) : Promise<Result<QmkConfigBlobChunkResponse, Error>> {
try {
...
async keymapGetKeycode(id: string, arg: KeymapGetKeycodeArg) : Promise<Result<KeymapGetKeycodeResponse, Error>> {
try {
...
async keymapGetEncoderKeycode(id: string, arg: KeymapGetEncoderKeycodeArg) : Promise<Result<KeymapGetEncoderKeycodeResponse, Error>> {
try {
    return { status: "ok", data: await TAURI_INVOKE("keymap_get_encoder_keycode", { id, arg }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async remappingGetLayerCount(id: string) : Promise<Result<RemappingGetLayerCountResponse, Error>> {
try {
...
async remappingSetKeycode(id: string, arg: RemappingSetKeycodeArg) : Promise<Result<null, Error>> {
try {
...
async remappingSetEncoderKeycode(id: string, arg: RemappingSetEncoderKeycodeArg) : Promise<Result<null, Error>> {
try {
    return { status: "ok", data: await TAURI_INVOKE("remapping_set_encoder_keycode", { id, arg }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async backlightGetEnabledEffects(id: string) : Promise<Result<BacklightGetEnabledEffectsResponse, Error>> {
try {
    return { status: "ok", data: await TAURI_INVOKE("backlight_get_enabled_effects", { id }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async rgblightGetEnabledEffects(id: string) : Promise<Result<RgblightGetEnabledEffectsResponse, Error>> {
try {
    return { status: "ok", data: await TAURI_INVOKE("rgblight_get_enabled_effects", { id }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async rgbmatrixGetEnabledEffects(id: string) : Promise<Result<RgbmatrixGetEnabledEffectsResponse, Error>> {
try {
    return { status: "ok", data: await TAURI_INVOKE("rgbmatrix_get_enabled_effects", { id }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type KeyCode = { code?: number; key: string; group?: string | null; label?: string | null; aliases?: string[] }
...
export type KeymapInfo = { layer_count: number | null; get_keycode_enabled: boolean; get_encoder_keycode_enabled: boolean }
export type KeymapKey = { code: KeyCode; position: Point3D }
export type Layout = { layout: LayoutEntry[] }
export type LayoutEntry = { matrix: Point2D; x: number; y: number; w?: number; h?: number; r?: number; rx?: number; ry?: number }
export type LightingCapabilities = { effects: LightingEffect[]; get_config_enabled: boolean; set_config_enabled: boolean; save_config_enabled: boolean }
...
export type LightingEffects = { groups: { [key in string]: LightingGroup } | null; effects: { [key in number]: LightingEffect } }
export type LightingGroup = { define: string }
export type LightingInfo = { backlight: LightingCapabilities | null; rgblight: LightingCapabilities | null; rgbmatrix: LightingCapabilities | null }
export type MappedKeymap = { keys: (((MappedKeymapKey | null)[])[])[]; dimensions: Point3D; size: Point2D }
...
export type QmkBoardIdentifiersResponse = { vendor_id: number; product_id: number; product_version: number; qmk_unique_identifier: number }
export type QmkBoardManufacturerResponse = UTF8String
export type QmkCapabilitiesFlags = number
export type QmkConfigBlobChunkResponse = [number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number]
export type QmkConfigBlobLengthResponse = number
export type QmkHardwareIdentifierResponse = [number, number, number, number]
export type QmkInfo = { version: string; board_ids: QmkBoardIdentifiersResponse; manufacturer: string; product_name: string; hardware_id: string; jump_to_bootloader_enabled: boolean; eeprom_reset_enabled: boolean }
...
export type RemapInfo = { layer_count: number | null; set_keycode_enabled: boolean; set_encoder_keycode_enabled: boolean }
export type RemappingCapabilitiesFlags = number
export type RemappingGetLayerCountResponse = number
export type RemappingSetEncoderKeycodeArg = { layer: number; encoder: number; clockwise: number; keycode: number }
 ...
 */
export type RgbLightConfig = { enable: number; mode: number; hue: number; sat: number; val: number; speed: number }
/**
 * RGB config for RGB matrix subsystem
 */
export type RgbMatrixConfig = { enable: number; mode: number; hue: number; sat: number; val: number; speed: number; flags: number }
...
export type XapConstants = { keycodes: XapKeyCodeCategory[]; rgblight_modes: LightingEffects; rgb_matrix_modes: LightingEffects; led_matrix_modes: LightingEffects }
export type XapDeviceInfo = { xap: XapInfo; qmk: QmkInfo; keymap: KeymapInfo | null; remap: RemapInfo | null; lighting: LightingInfo | null }
export type XapDeviceState = { id: string; info: XapDeviceInfo | null; config: Config; secure_status: XapSecureStatus }
export type XapEnabledSubsystemCapabilitiesFlags = number
export type XapEvent = { kind: "LogReceived"; data: { id: string; log: string } } | { kind: "SecureStatusChanged"; data: { id: string; secure_status: XapSecureStatus } } | { kind: "NewDevice"; data: { id: string } } | { kind: "RemovedDevice"; data: { id: string } }
